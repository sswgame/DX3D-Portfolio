                              ...
                              void*   _pInitialData)
{
	...
	m_desc.BindFlags           = (SB_TYPE::READ_ONLY == m_eType)
...
void CStructuredBuffer::SetData(const void* _pSrc, UINT _iElementCount) const
{
	assert(m_bCpuAccess);
...
void CStructuredBuffer::UpdateData_CS(UINT _iRegisterNum, bool _bShaderResource)
{
	...
	else
	{
		...
		constexpr UINT invalidRegisterNumber = STRUCTURED_BUFFER::INVALID_INDEX;
		CONTEXT->CSSetUnorderedAccessViews(_iRegisterNum, 1, m_UAV.GetAddressOf(), &invalidRegisterNumber);
	}
}

void CStructuredBuffer::Clear()
{
	if (STRUCTURED_BUFFER::INVALID_INDEX != m_iRecentBindNumSRV)
	{
		ID3D11ShaderResourceView* pNullSRV = nullptr;
		CONTEXT->VSSetShaderResources(m_iRecentBindNumSRV, 1, &pNullSRV);
		CONTEXT->HSSetShaderResources(m_iRecentBindNumSRV, 1, &pNullSRV);
		CONTEXT->DSSetShaderResources(m_iRecentBindNumSRV, 1, &pNullSRV);
		CONTEXT->GSSetShaderResources(m_iRecentBindNumSRV, 1, &pNullSRV);
		CONTEXT->PSSetShaderResources(m_iRecentBindNumSRV, 1, &pNullSRV);
		CONTEXT->CSSetShaderResources(m_iRecentBindNumSRV, 1, &pNullSRV);
		m_iRecentBindNumSRV = STRUCTURED_BUFFER::INVALID_INDEX;
	}

	if (STRUCTURED_BUFFER::INVALID_INDEX != m_iRecentBindNumUAV)
	{
		ID3D11UnorderedAccessView* pNullUAV              = nullptr;
		constexpr UINT             invalidRegisterNumber = STRUCTURED_BUFFER::INVALID_INDEX;
		CONTEXT->CSSetUnorderedAccessViews(m_iRecentBindNumUAV, 1, &pNullUAV, &invalidRegisterNumber);
		m_iRecentBindNumUAV = STRUCTURED_BUFFER::INVALID_INDEX;
	}
}
