...
Vec4 CFBXLoader::GetMtrlData(FbxSurfaceMaterial* _pSurface, const char* _pMtrlName, const char* _pMtrlFactorName)
{
	...
	Vec4 vRetVal = Vec4((float)vMtrl.mData[0] * (float)dFactor,
...
void CFBXLoader::LoadAnimationClip()
{
	...
	for (int i = 0; i < iAnimCount; ++i)
	{
		...
		pAnimClip->llTimeLength = pAnimClip->tEndTime.GetFrameCount(pAnimClip->eMode) - pAnimClip->tStartTime.
		                          GetFrameCount(pAnimClip->eMode);
...
void CFBXLoader::LoadKeyframeTransform(FbxNode*          _pNode,
