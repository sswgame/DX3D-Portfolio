...
{
	emitter << YAML::Key << "m_eProjType" << YAML::Value << (int)m_eProjType;
	emitter << YAML::Key << "m_fWidth" << YAML::Value << m_fWidth;
	emitter << YAML::Key << "m_fAspectRatio" << YAML::Value << m_fAspectRatio;
	emitter << YAML::Key << "m_fFOV" << YAML::Value << m_fFOV;
	emitter << YAML::Key << "m_fFar" << YAML::Value << m_fFar;
	emitter << YAML::Key << "m_iLayerMask" << YAML::Value << m_iLayerMask;
	emitter << YAML::Key << "m_iCamIdx" << YAML::Value << m_iCamIdx;
}

void CCamera::Deserialize(const YAML::Node& node)
{
	m_eProjType    = (PROJ_TYPE)node["m_eProjType"].as<int>();
	m_fWidth       = node["m_fWidth"].as<float>();
	m_fAspectRatio = node["m_fAspectRatio"].as<float>();
	m_fFOV         = node["m_fFOV"].as<float>();
	m_fFar         = node["m_fFar"].as<float>();
	m_iLayerMask   = node["m_iLayerMask"].as<UINT>();
	m_iCamIdx      = node["m_iCamIdx"].as<int>();
}
