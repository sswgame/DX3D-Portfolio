...
{
	emitter << YAML::Key << "m_eColliderType" << YAML::Value << (int)m_eColliderType;
	emitter << YAML::Key << "m_vOffsetPos" << YAML::Value << m_vOffsetPos;
	emitter << YAML::Key << "m_vOffsetScale" << YAML::Value << m_vOffsetScale;
}

void CCollider2D::Deserialize(const YAML::Node& node)
{
	m_eColliderType = (COLLIDER2D_TYPE)node["m_eColliderType"].as<int>();
	m_vOffsetPos    = node["m_vOffsetPos"].as<Vec2>();
	m_vOffsetScale  = node["m_vOffsetScale"].as<Vec2>();
}
