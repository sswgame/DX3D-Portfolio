...
void CTestScene::CreateTestScene()
{
	...
	pCurScene->AddObject(pLight3D, L"Default");#1#
	...
	pObject->Transform()->SetRelativeScale(1000.f, 2000.f, 1000.f);
	pObject->Transform()->SetRelativeRotation(0.f, 0.f, 0.f);

	pObject->LandScape()->SetDynamicShadow(true);
	...
	CScene*      pScene      = CSceneSaveLoad::LoadScene(contentPath + L"scene\\TestScene.scene");
	pScene->start();
	pScene->SetSceneState(SCENE_STATE::STOP);
